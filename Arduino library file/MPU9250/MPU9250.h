...
class MPU9250_ {
    ...
    float magnetic_declination = -7.15;  // Japan, 24th June
...
public:
    ...
    bool update() {
        ...
        if (!b_ahrs) {
            ...
            // Self Addition
            // rotation matrix full version--------------------------------------------------------------------------------
            r11 = 1.0f - 2.0f*pow(q[2], 2) - 2.0f*pow(q[3], 2);
            r12 = 2.0f * (q[1] * q[2] - q[3] * q[0]);
            r13 = 2.0f * (q[1] * q[3] + q[2] * q[0]);
            r21 = 2.0f * (q[1] * q[2] + q[3] * q[0]);
            r22 = 1.0f - 2.0f*pow(q[1], 2) - 2.0f*pow(q[3], 2);
            r23 = 2.0f * (q[2] * q[3] - q[1] * q[0]);
            r31 = 2.0f * (q[1] * q[3] - q[2] * q[0]);
            r32 = 2.0f * (q[2] * q[3] + q[1] * q[0]);
            r33 = 1.0f - 2.0f*pow(q[1], 2) - 2.0f*pow(q[2], 2);
            // ------------------------------------------------------------------------------------------------------------
    ...
    float getMagOffsetX() const { return mag_bias_factory[0]; }
    float getMagOffsetY() const { return mag_bias_factory[1]; }
    float getMagOffsetZ() const { return mag_bias_factory[2]; }

    float getTemperature() const { return temperature; }

    // Self Addition
    // Getting rotation matrix value---------------------------------------------------------------------------------------
    float getR11() const { return r11; }
    float getR12() const { return r12; }
    float getR13() const { return r13; }
    float getR21() const { return r21; }
    float getR22() const { return r22; }
    float getR23() const { return r23; }
    float getR31() const { return r31; }
    float getR32() const { return r32; }
    float getR33() const { return r33; }
    //---------------------------------------------------------------------------------------------------------------------
    ...
    void setMagOffset(const float x, const float y, const float z) {
        mag_bias_factory[0] = x;
        mag_bias_factory[1] = y;
        mag_bias_factory[3] = z;
    }
