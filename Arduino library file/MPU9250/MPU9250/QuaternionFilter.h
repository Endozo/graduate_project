...
class QuaternionFilter {
    ...
    // for mahony
    float Kp = 0.00008; // 0.00023; // 0.00008; // 0.002;
    float Ki = 0.0;

    QuatFilterSel filter_sel{QuatFilterSel::MAHONY};  // set the algorithm mode to MAHONY
    double deltaT{0.};
...
public:
    ...
    void mahony(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz, float* q) {
        ...
        if ( (mx==0.0f) && (my==0.0f) && (mz==0.0f) ) mode = 1;
        else mode = 2;

        switch (mode) {
            case 1: // if we can only get acc and gyro value
                ...
                if (tmp > 0.0) {
                    ...
                    if (Ki > 0.0f) {
                        ix += 2*Ki * ex * deltaT;  // integral error scaled by Ki
                        iy += 2*Ki * ey * deltaT;
                        iz += 2*Ki * ez * deltaT;
            ...
            case 2:
                ...
                if(Ki > 0.0f) {
                    ix += 2*Ki * halfex * deltaT;  // integral error scaled by Ki
                    iy += 2*Ki * halfey * deltaT;
                    iz += 2*Ki * halfez * deltaT;
                    gx += ix;  // apply integral feedback
                    gy += iy;
                    gz += iz;
                }
